...
Inception は 42 のシステム管理プロジェクトです。目的は、Docker と Docker Compose を使って、小さな Web インフラを自分で構築することです。サービスごとに独自の Docker image を作り、設定、起動処理、永続化、ネットワークを明確に分けています。
...
Docker Engine と Docker Compose がインストールされている必要があります。

### 1. ホスト側データディレクトリを作成する
